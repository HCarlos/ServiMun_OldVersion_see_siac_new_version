            ...
            <table  id="tblCat" class="table table-bordered table-striped dt-responsive nowrap dataTable " role="grid" aria-describedby="datatable-buttons_info" style="width: 100%; position: relative; z-index:0;" width="100%">
                <thead>
                    <tr role="row">
                        ...
                        <th class="sorting">Familia</th>
                        {{--<th class="sorting">Emails</th>--}}
                ...
                @foreach($items as $item)
                    <tr>
                        ...
                        <td>{{($item->familia)}}
                            <div class="badge badge-secondary">{{$item->padres->count()}}</div>
                            <div class="badge badge-cafe">{{$item->rfcs->count()}}</div>
                            <div class="badge badge-dark">{{$item->hijos->count()}}</div>
                        </td>
                        {{--<td>{{($item->emails)}}</td>--}}
                        <td class="table-action">
                            <div class="button-list">
                                ...
                                @include('shared.ui_kit.__list_fam_parent')
                                @include('shared.ui_kit.__add_item_regfis')
                                @include('shared.ui_kit.__add_item_hijo')
                            </div>
                        </td>
                    </tr>
                @endforeach
